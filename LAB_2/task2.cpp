...
void t2(double x_step, double t_step, double t_fin, std::string out_filename) {
	int N = 1 / x_step;
	int L = t_fin / t_step;
	...
	for (int i = 1; i < N; i++) {
		a_coef[i] = 1;
		b_coef[i] = 2 + x_step * x_step / t_step;
		c_coef[i] = 1;
	}
	...
	for (int j = 0; j < L; j++) {
		...
		for (int i = 1; i < N; i++) {
			double x_i = i * x_step;
			d_coef[i] = -u[j][i] * x_step * x_step / t_step - x_step * x_step * func(x_i, j * t_step);
		}
